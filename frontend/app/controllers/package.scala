...
package object controllers {
  ...
  case class UserInfo(username: String, id: Long, isAdmin: Boolean)
  ...
  class UserInfoAction @Inject()(sessionService: SessionService,
    ...
    def authorizedUserAction(checkAuthorization: Option[UserInfo] => Boolean)
                            (block: UserRequest[_] => Future[Result]): Action[AnyContent] = {
      async { implicit userRequest: UserRequest[_] =>
        if (checkAuthorization(userRequest.userInfo)) {
          block(userRequest)
        } else {
          Future.successful {
            Redirect(routes.HomeController.index()).flashing(FLASH_ERROR -> "Not authorized!")
          }
        }
      }
    }
